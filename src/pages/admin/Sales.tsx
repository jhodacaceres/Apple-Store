...
import { Download, Plus, Eye, X, ShoppingBag, Pencil, Trash2, RotateCcw, AlertTriangle } from 'lucide-react';
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm">
          <div className={`${modalBg} rounded-3xl shadow-2xl w-full max-w-md overflow-hidden`}>
            ...
            <div className="p-6 space-y-4">
              ...
              <div>
                <label className="block text-xs font-semibold text-gray-400 mb-1.5">Total</label>
          ...
          <table className="w-full text-left text-sm min-w-[760px]">
            <thead>
              <tr className={`border-b ${dark ? 'border-gray-700' : 'border-gray-100'}`}>
                {['ID Pedido', 'Fecha', 'Cliente', 'Vendedor', 'Producto', 'Tipo', 'Total', 'Estado', 'Acciones'].map((h) => (
        ...
        <div className={cardClass}>
          {deletedOrders.length === 0 ? (
            ...
            <table className="w-full text-left text-sm min-w-[760px]">
              <thead>
                <tr className={`border-b ${dark ? 'border-gray-700' : 'border-gray-100'}`}>
                  {['ID Pedido', 'Fecha', 'Cliente', 'Producto', 'Total', 'Eliminada', 'Acciones'].map((h) => (
                    <th key={h} className={thClass}>{h}</th>
                  ))}
                </tr>
              </thead>
